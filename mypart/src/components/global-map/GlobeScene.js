...
export class GlobeScene {
  ...
  buildTargetNodes() {
    ...
    this.targetNodesData.forEach((node) => {
      ...
      const pinGeo = new THREE.CylinderGeometry(0.08, 0.03, 0.35, 16);
  ...
  animate() {
    ...
    if (this.globeState === "EXPLODED") {
      for (let i = 0; i < posAttr.count; i++) {
        ...
        if (this.stateTimer < 0.8) {
          ...
          posAttr.array[i3] =
            this.originalPositions[i3] + this.randomDirections[i3] * shift;
          posAttr.array[i3 + 1] =
            this.originalPositions[i3 + 1] +
            this.randomDirections[i3 + 1] * shift;
          posAttr.array[i3 + 2] =
            this.originalPositions[i3 + 2] +
            this.randomDirections[i3 + 2] * shift;
        } else {
    ...
    this.nodeMeshes.forEach((node) => {
      const scaleValue =
        node.baseScale * (1 + Math.sin(elapsedTime * node.pulseSpeed) * 0.35);
