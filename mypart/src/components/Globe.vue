...
watch(globalSources, (newSources) => {
  if (!globeEngineInstance) return
  const statuses = {}
  newSources.forEach(s => {
    statuses[s.name] = s.lastScrape && s.lastScrape.includes('Success')
  })
  ...
  nextTick(() => {
    if (globeCanvas.value) {
      ...
      setTimeout(() => {
        handleResize()
        const statuses = {}
        if (globalSources && globalSources.value) {
          globalSources.value.forEach(s => {
            statuses[s.name] = s.lastScrape && s.lastScrape.includes('Success')
          })
          globeEngineInstance.updateFromData(statuses)
        }
